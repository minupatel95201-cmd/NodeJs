...
module.exports.forgetPassword = async (email) => {
    ...
    // token will be validate only 15 minutes, after 15 mi nutes token will be expiry and you can't reset your password
    await user.save();

    const resetLink = `http://localhost:3500/reset-password/${token}`
    //resetLink = frontend pade link that show newPassword filed with change password btn
...
The Support Team</p>
    ...
    module.exports.resetPassword = async ({ token, newPassword }) => {
        ...
        if(!user){
            throw new Error ("Link Expire, Send New Reuqest");
        };
    };
